...
class Funcoes {
	...
	/**
	 * Os únicos dias cadastrados no BD são seg, ter, qua e qui. Quando um aluno vai para a página do cardápio, ele carrega o cardápio conforme o dia em que o aluno está. Se o aluno, então, entrar no final de semana, ocorrerrá um erro. Essa função corrige esse problema: ela recebe uma data e, se for um final de semana, retorna a data do intervalo de 4 dias mais próxima.
	...
	public static function CorrigeData (string $data) {
		...
		if (in_array(date("w", strtotime($data)), $dias_com_almoco)) { // se o dia da semana da data informada é um desses quatro
			return date("Y-m-d", $data); // retorna a data normalmente
		} else {
