...
if ($acao == 'AlterarSenha') {
  ...
  $senhaAntiga = sha1($_POST['senhaAntiga']); $_POST['senhaAntiga'] = '';
  $senhaNova = sha1($_POST['senhaNova']); $_POST['senhaNova'] = '';

  $usuario = UsuarioDao::SelectPorMatricula($usuario->getCodigo());
  if ($usuario->getSenha() == $senhaAntiga) {
    $usuario->setSenha( $senhaNova );
    UsuarioDao::Update($usuario);
    header("location:".$root_path."aluno/perfil/?sucesso=alteracao_senha"); // + retorno de sucesso
  } else {
